...
from predict_final import enter_stock, prep_data, prep_model, test_models, predict_dtr_plot
import numpy as np
from tkinter import Tk
from tkinter.filedialog import askopenfilename, askdirectory

# y_data=np.array(predictions[-days_predict:])
# x_data=np.array(x[-days_predict:])
...
def user_input():
    ...
    ticker = ""
    while ticker != "-1":
        ticker = input("Enter a ticker or -1 to exit: ")
        if ticker != "-1":
...
def predict(list_of_tickers, days_to_predict, csv_bool, png_bool, filepath):
    ...
    for ticker in list_of_tickers:  # We predict using data going back to January 1st, 2000
        ...
        stock_df = prep_data(data)
        try:

            # Export PART 2 BEGIN#
            x, y, x_train, x_validation, y_train, y_validation = prep_model(stock_df)
            test_models(ticker, x_train, y_train)
            if png_bool:
                predictions, dates = predict_dtr_plot(ticker, x, y, x_train, y_train, int(days_to_predict), file_path=filepath)
            elif not png_bool:
                predictions, dates = predict_dtr_plot(ticker, x, y, x_train, y_train, int(days_to_predict), file_path=filepath)
            else:
                predictions = None
            ...
            if csv_bool and predictions.any() and dates:
                try:
                    ...
                    data = pd.DataFrame({"Open": predictions, "Date": dates})
    ...
    if start.lower() == 'y':
        ...
        if export.lower() == 'y':
            ...
            if png.lower() == 'y':
                ...
                predict(tickers, days, csv_bool=True, png_bool=False, filepath=None)
        if export.lower() == 'n':
